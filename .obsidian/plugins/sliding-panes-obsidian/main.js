...
var SlidingPanesSettingTab = /** @class */ (function (_super) {
  ...
  SlidingPanesSettingTab.prototype.display = function () {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle(function (toggle) {
        return toggle
          .setValue(_this.plugin.settings.smoothAnimation)
          .onChange(function (value) {
...
var SlidingPanesCommands = /** @class */ (function () {
  ...
  SlidingPanesCommands.prototype.addToggleSettingCommand = function (
    id,
    name,
    settingName
  ) {
    ...
    this.plugin.addCommand({
      ...
      callback: function () {
        ...
        //@ts-ignore
        _this.plugin.settings[settingName] =
          !_this.plugin.settings[settingName];
        _this.plugin.saveData(_this.plugin.settings);
        _this.plugin.refresh();
      },
  ...
  SlidingPanesCommands.prototype.addCommands = function () {
    ...
    this.plugin.addCommand({
      ...
      callback: function () {
        ...
        _this.plugin.settings.disabled
          ? _this.plugin.disable()
          : _this.plugin.enable();
      },
    });
    ...
    this.addToggleSettingCommand(
      "toggle-sliding-panes-leaf-auto-width",
      "Toggle Leaf Auto Width",
      "leafAutoWidth"
    );
    // add a command to toggle stacking
    this.addToggleSettingCommand(
      "toggle-sliding-panes-stacking",
      "Toggle Stacking",
      "stackingEnabled"
    );
    // add a command to toggle rotated headers
    this.addToggleSettingCommand(
      "toggle-sliding-panes-rotated-headers",
      "Toggle Rotated Headers",
      "rotateHeaders"
    );
    // add a command to toggle swapped header direction
    this.addToggleSettingCommand(
      "toggle-sliding-panes-header-alt",
      "Swap rotated header direction",
      "headerAlt"
    );
  };
    ...
    get: function () {
      if (!this._cachedClassName) {
        ...
        if (this._cachedClassName.endsWith("-obsidian")) {
          this._cachedClassName = this._cachedClassName.substring(
            0,
            this._cachedClassName.lastIndexOf("-obsidian")
          );
        }
      }
...
var SlidingPanesPlugin = /** @class */ (function (_super) {
  ...
  function SlidingPanesPlugin() {
    ...
    _this.enable = function () {
      var _a;
      if (
        !((_a = _this.settings) === null || _a === void 0
          ? void 0
          : _a.disabled)
      ) {
        // add the event handlers
        _this.registerEvent(
          _this.app.workspace.on("resize", _this.handleResize)
        );
        _this.registerEvent(
          _this.app.workspace.on("layout-change", _this.handleLayoutChange)
        );
        _this.registerEvent(
          _this.app.workspace.on(
            "active-leaf-change",
            _this.handleActiveLeafChange
          )
        );
        _this.registerEvent(
          _this.app.workspace.on("window-open", _this.handleWindowOpen)
        );
        _this.registerEvent(_this.app.vault.on("delete", _this.handleDelete));
        // wait for layout to be ready to perform the rest
        if (_this.app.workspace.layoutReady) _this.reallyEnable();
      }
    };
    ...
    _this.clearLeaf = function (leaf) {
      ...
      if (iconText.includes("(")) {
        iconEl.setAttribute(
          "aria-label",
          iconText.substring(
            iconText.lastIndexOf("(") + 1,
            iconText.lastIndexOf(")")
          )
        );
      }
    };
    ...
    _this.updateStyle = function () {
      ...
      if (!el) throw "plugin-sliding-panes element not found!";
        ...
        el.innerText = "body.plugin-sliding-panes{--header-width:".concat(
        ...
        if (!_this.settings.leafAutoWidth) {
          if (obsidian.Platform.isDesktop) {
            el.innerText +=
              "body.plugin-sliding-panes .mod-root>.workspace-leaf,body.plugin-sliding-panes .mod-root>.workspace-split{width:".concat(
                _this.settings.leafDesktopWidth + _this.settings.headerWidth,
                "px;}"
              );
          } else {
            el.innerText +=
              "body.plugin-sliding-panes .mod-root>.workspace-leaf,body.plugin-sliding-panes .mod-root>.workspace-split{width:".concat(
                _this.settings.leafMobileWidth + _this.settings.headerWidth,
                "px;}"
              );
          }
        }
          ...
          var mouseup_1 = function () {
            ...
            if (_this.settings.stackingEnabled) {
              ...
              for (var i = leafIndex; i < leafCount; i++) {
                rootLeaves[i].containerEl.style.right =
    ...
    _this.recalculateLeaves = function () {
      ...
      rootSplits.forEach(function (rootSplit) {
        ...
        var leafWidth = _this.settings.leafAutoWidth
          ...
          ? _this.settings.leafDesktopWidth
          : _this.settings.leafMobileWidth;
        ...
        rootLeaves.forEach(function (leaf, i) {
          ...
          if (totalWidthEstimate > rootContainerElWidth) {
            ...
            if (_this.settings.stackingEnabled) {
              ...
              right =
                (leafCount - i) * _this.settings.headerWidth - leafWidth + "px";
            }
          }
          // set the html attributes for the leaf (if they have changed)
          if (
            containerEl.style.flex != flex ||
            containerEl.style.width != width + "px" ||
            containerEl.style.left != left ||
            containerEl.style.right != right
          ) {
          ...
          if (leaf instanceof obsidian.WorkspaceLeaf) {
            ...
            if (!iconText.includes("(")) {
              iconEl.setAttribute(
                "aria-label",
                "".concat(leaf.getDisplayText(), " (").concat(iconText, ")")
              );
            }
          }
        });
  ...
  SlidingPanesPlugin.prototype.focusLeaf = function (activeLeaf, animated) {
    ...
    while (
      ...
      activeLeaf.parentSplit !== rootSplit
    ) {
    ...
    if (
      ...
      var behavior =
        animated && this.settings.smoothAnimation ? "smooth" : "auto";
      // it's too far left
